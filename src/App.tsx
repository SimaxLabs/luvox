...
function FieldLabel({ action, children, htmlFor, optional }: { action?: ReactNode; children: ReactNode; htmlFor: string; optional?: boolean }) {
  return (
...
function PromptLibrary({
  ...
  const settingsLabels: Record<SavedPromptSettings["kind"], string> = {
    image: "OpenRouter image",
    mflux: "MFLUX image",
    openrouter: "OpenRouter video",
    local: "h3.c video",
  };
  ...
  return (
    <>
      <button
        ...
        className="border border-black/20 px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.12em] text-stone-600 transition hover:border-black hover:bg-black hover:text-[#d9ff72] disabled:cursor-not-allowed disabled:opacity-40"
        disabled={disabled}
      ...
      <dialog
        ...
        className="m-0 h-[100dvh] max-h-none w-full max-w-none bg-transparent p-0 text-[#191b18] backdrop:bg-black/80 sm:m-auto sm:h-auto sm:max-h-[90dvh] sm:w-[min(56rem,calc(100vw-3rem))]"
        ref={dialog}
      >
        <div className="flex h-full max-h-[100dvh] flex-col bg-[#f0efe8] shadow-2xl sm:max-h-[90dvh]">
          ...
          <fieldset className="grid min-h-0 flex-1 overflow-y-auto border-0 p-0 lg:grid-cols-[0.8fr_1.2fr] lg:overflow-hidden" disabled={disabled}>
            <section className="border-b border-black/10 p-5 sm:p-7 lg:overflow-y-auto lg:border-b-0 lg:border-r">
              ...
              <input
                className="h-11 w-full border border-black/15 bg-[#faf9f3] px-3 text-sm outline-none transition focus:border-black focus:ring-2 focus:ring-[#d9ff72]"
                ...
                value={name}
              />
              <label className="mt-4 flex cursor-pointer items-center gap-2 text-xs text-stone-700">
                <input checked={includeSettings} onChange={(event) => setIncludeSettings(event.target.checked)} type="checkbox" />
                Include the current model and settings
              </label>
              <button
            ...
            <section className="p-5 sm:p-7 lg:overflow-y-auto">
              ...
              {prompts.length > 0 ? (
                    ...
                    <li className="border border-black/10 bg-[#faf9f3] p-3" key={saved.id}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate text-xs font-bold">{saved.name}</p>
                          <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-stone-500">{saved.prompt}</p>
                          <p className="mt-2 font-mono text-[9px] uppercase tracking-[0.1em] text-stone-500">
                            {saved.settings ? settingsLabels[saved.settings.kind] : "Prompt only"}
                          </p>
                        </div>
                        <div className="flex shrink-0 gap-2">
                          <button
                            className="border border-black/20 px-3 py-2 text-[9px] font-bold uppercase tracking-[0.12em] hover:border-black hover:bg-black hover:text-[#d9ff72]"
                            onClick={() => {
                              setStatus(onLoad(saved));
                ...
                <div className="flex min-h-48 items-center justify-center border border-dashed border-black/20 px-6 text-center text-xs leading-5 text-stone-500">No saved prompts yet.</div>
              )}
            </section>
          </fieldset>
        </div>
...
export default function App() {
  ...
  const currentPromptSettings: SavedPromptSettings = currentSubmissionKind === "image"
    ? {
        kind: "image",
        model: selectedOpenRouterImageModel.id,
        aspectRatio: openRouterImageAspectRatio,
        resolution: openRouterImageResolution,
      }
    : currentSubmissionKind === "mflux"
      ? {
          kind: "mflux",
          ...
          guidance: mfluxGuidance,
          referenceFit: imageReferenceFit,
        }
      : currentSubmissionKind === "openrouter"
        ? {
            kind: "openrouter",
  ...
  const loadSavedPrompt = (saved: SavedPrompt): string => {
    leaveJobView();
    setError(null);
    let status = `${saved.name} loaded.`;

    if (!saved.settings) {
      if (workflow === "image") setImagePrompt(saved.prompt);
      else setForm((current) => ({ ...current, prompt: saved.prompt }));
      setJobAnnouncement(status);
      return status;
    }
    ...
    if (settings.kind === "image") {
      ...
      const model = openRouterRegistry.images.find((candidate) => candidate.id === settings.model);
      if (model) {
        setOpenRouterImageModelId(model.id);
        setOpenRouterImageAspectRatio(model.aspectRatios.includes(settings.aspectRatio) ? settings.aspectRatio : model.defaultAspectRatio || "");
        setOpenRouterImageResolution(model.resolutions.includes(settings.resolution) ? settings.resolution : model.defaultResolution || "");
        setImageReference(null);
      } else {
        status = `${saved.name} loaded, but its saved OpenRouter image model is no longer available.`;
      }
    } else if (settings.kind === "mflux") {
      setWorkflow("image");
      setImagePrompt(saved.prompt);
      const model = getMfluxImageModel(settings.model);
      if (appConfig?.localMflux.supported !== true || !model) {
        setImageProvider("openrouter");
        status = `${saved.name} loaded, but its saved MFLUX settings are unavailable here.`;
      } else {
        const setup = MFLUX_IMAGE_RECOMMENDED_SETUPS.find((candidate) => (candidate.models as readonly string[]).includes(model.id)) ?? defaultMfluxSetup;
        setImageProvider("mflux");
        setMfluxModel(model.id);
        setMfluxResolution(getMfluxImageResolution(settings.resolution) ? settings.resolution : setup.resolution);
        setMfluxSteps(model.steps.some((steps) => steps === settings.steps) ? settings.steps : setup.steps);
        setMfluxQuantization(MFLUX_IMAGE_QUANTIZATIONS.some((value) => value === settings.quantization) ? settings.quantization : setup.quantization);
        setMfluxSeed(settings.seed);
        setMfluxLowRam(settings.lowRam);
        setMfluxVaeTiling(settings.vaeTiling);
        setMfluxVaeTileSize(MFLUX_VAE_TILE_SIZES.some((size) => size === settings.vaeTileSize) ? settings.vaeTileSize : setup.vaeTileSize);
        setMfluxGuidance(settings.guidance);
        setImageReferenceFit(settings.referenceFit);
        setImageReference(null);
        setMfluxAdvancedStatus("Saved settings applied.");
      }
    } else if (settings.kind === "openrouter") {
      setWorkflow("video");
      setForm((current) => ({ ...current, provider: "openrouter", prompt: saved.prompt }));
      const model = openRouterRegistry.videos.find((candidate) => candidate.id === settings.model);
      if (model) {
        setForm((current) => ({
          ...current,
          provider: "openrouter",
          prompt: saved.prompt,
          model: model.id,
          duration: model.durations.includes(settings.duration) ? settings.duration : model.defaultDuration,
          aspectRatio: model.aspectRatios.includes(settings.aspectRatio) ? settings.aspectRatio : model.defaultAspectRatio,
          resolution: model.resolutions.includes(settings.resolution) ? settings.resolution : model.defaultResolution || "",
          firstFrameUrl: "",
          lastFrameUrl: "",
          generateAudio: model.generateAudio.supported ? settings.generateAudio : model.generateAudio.default,
        }));
      } else {
        status = `${saved.name} loaded, but its saved OpenRouter video model is no longer available.`;
      }
    } else {
      setWorkflow("video");
      if (appConfig?.localH3.supported !== true) {
        setForm((current) => ({ ...current, provider: "openrouter", prompt: saved.prompt }));
        status = `${saved.name} loaded, but its saved h3.c settings are unavailable here.`;
      } else {
        const resolution = LOCAL_H3_RESOLUTIONS.some((candidate) => candidate.id === settings.resolution) ? settings.resolution : "512x512";
        const frames = LOCAL_H3_DURATIONS.some((candidate) => candidate.frames === settings.frames) ? settings.frames : 22;
        const quality = LOCAL_H3_QUALITY_PRESETS.some((candidate) => candidate.id === settings.quality) ? settings.quality : "balanced";
        const acceleration = isLocalH3AccelerationAvailable(settings.acceleration, resolution, quality) ? settings.acceleration : "standard";
        for (const token of new Set(Object.values(referenceUploadTokens))) {
          if (token) void deleteLocalReferenceImage(token, localWorkspaceToken.current).catch(() => undefined);
        }
        setReferenceUploadTokens({});
        setReferenceUploadStatus("");
        firstFramePathOnFocus.current = "";
        ...
        setForm((current) => ({
          ...
          localFirstFramePath: "",
          localLastFramePath: "",
          localFrameFit: settings.frameFit,
          localSsdStreaming: settings.ssdStreaming,
        }));
        setLocalAdvancedStatus("Saved settings applied.");
      }
    }

    if (status !== `${saved.name} loaded.`) setError(status);
    setJobAnnouncement(status);
    return status;
  };
              ...
              <FieldLabel
                action={(
                  <PromptLibrary
                    ...
                    currentSettings={currentPromptSettings}
                    disabled={readingImageReference || uploadingReference !== null || clearingWorkspace}
                    onLoad={loadSavedPrompt}
                  />
                )}
                htmlFor="image-prompt"
              >
                {usesMfluxEdit ? "Edit instruction" : "Image prompt"}
              </FieldLabel>
