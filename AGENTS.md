...
- Preview and download must stay behind `/api/video/content/:id`; that server proxy attaches authentication and forwards range headers. Temporary-key jobs use their pinned key, while server-key jobs use an independent volatile per-job capability in `X-Motio-Video-Token` with a 24-hour idle expiry. The client fetches protected media and creates a revocable `blob:` URL. Never put either credential in media URLs or assume `unsigned_urls` are permanently public.
...
- `server/localMflux.ts` must spawn with an argument array and `shell: false`, use owner-marked server temporary paths, cap output, return generic engine errors, and avoid passing the full server environment. Prompts use `--prompt-file` so they do not appear in process listings. Remove original references after framing; startup may delete only stale directories bearing Motio's strict ownership marker.
