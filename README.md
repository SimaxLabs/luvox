...
The backend runs one generation at a time and accepts at most three waiting jobs. The UI exposes resolution, duration, quality, seed, first/last reference frames, and optional SSD streaming presets. Outputs remain in `H3_JOBS_DIR`; queued and processing state is held in server memory, while completed output can still be served after a restart using its persisted job ID.
