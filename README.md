...
The backend runs one generation at a time and accepts at most three waiting jobs. The local presets include exact `576x1024` (`9:16`) output and h3.c's documented 22, 39, 56, 107, 243, and 362-frame durations. The UI also exposes quality, seed, first/last reference frames, and optional SSD streaming presets. Outputs remain in `H3_JOBS_DIR`; queued and processing state is held in server memory, while completed output can still be served after a restart using its persisted job ID.

Local first/last frames can be entered as absolute paths to readable PNG, JPEG, or WebP files up to 25 MB. The Browse buttons upload a selected image to `H3_JOBS_DIR/reference-images` and fill that server-owned absolute path into the form. Browsers do not reveal the selected file's original absolute path. Each submitted job validates, copies, and frames its references to the selected output canvas before entering the queue. **Preserve full image** is the default: it keeps every source pixel in proportion and pads the extra canvas required by a different output ratio. **Crop to fill** is an explicit alternative that removes image edges. Both avoid h3.c's native first-frame stretching. Staged uploads expire after 24 hours; the staging area accepts up to 20 unexpired images and never deletes a newer selection to admit another upload.
